RMenu.Add('showcase', 'iProMxMenu', RageUI.CreateMenu("Hypnotisation", "~r~By TheZeRoX", 8, 200, nil, nil, 213, 170, 115))
RMenu:Get('showcase', 'iProMxMenu').EnableMouse = false

RMenu.Add('showcase', 'submenu1', RageUI.CreateSubMenu(RMenu:Get('showcase', 'iProMxMenu'), "Hypnotisation", "Hypnotisation"))
RMenu.Add('showcase', 'submenu2', RageUI.CreateSubMenu(RMenu:Get('showcase', 'iProMxMenu'), "Troll", "Troll"))
...
RageUI.CreateWhile(1.0, function()
                ...
                if (Selected) then
                    ...
                    if closestPlayer == -1 or closestDistance > 15.0 then 
                        ...
                        if not PouvoirON1 then
                            ...
                            TriggerServerEvent('esx_pouvoir:hypno__')
                        else
                            print('else')
                        end
                    end
    ...
    if RageUI.Visible(RMenu:Get('showcase', 'submenu2')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()
            ...
            RageUI.Button("Forcer le joueur à dire que TheZerox est BG ", nil, { RightLabel = "😂" }, true, function(_, _, Selected)
                if (Selected) then
                    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
            ...
            RageUI.Button("Forcer le joueur à dire que Damien est BG ", nil, { RightLabel = "😂😂" }, true, function(_, _, Selected)
                if (Selected) then
                    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                    ...
                    if closestPlayer == -1 or closestDistance > 2.0 then 
                        ...
                        if not PouvoirON1 then
                            TriggerServerEvent('esx_pouvoir:sendclosest', GetPlayerServerId(closestPlayer))
                         ESX.ShowNotification(GetPlayerName(closestPlayer) ..' est sous vos pouvoirs ~w~')
                            TriggerServerEvent('esx_pouvoir:closestBGhypno')
                        else
                            ESX.ShowNotification('Ce joueur fais deja une animation')
                        end
                    end
            ...
            RageUI.Button("Forcer le joueur à tomber sous le charme de Damien ", nil, { RightLabel = "😂" }, true, function(_, _, Selected)
                if (Selected) then
                    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
            ...
            RageUI.Button("Forcer le joueur à vous suivre ", " le roi arouf", { RightLabel = "←←←" }, true, function(_, _, Selected)
                if (Selected) then
                    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                    if closestPlayer == -1 or closestDistance > 2.0 then 
                        ESX.ShowNotification('~r~Il n\'y a pas de joueur à proximité')
            ...
            RageUI.Button("Cloner le personnage devant vous ", "le roi arouf", { RightLabel = "←←←" }, true, function(_, _, Selected)
                if (Selected) then
                    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
            ...
            RageUI.Button("Cloner votre personnage ", "le roi arouf", { RightLabel = "←←←" }, true, function(_, _, Selected)
                if (Selected) then
            ...
            RageUI.Button("Cloner le personnage et le forcer a vous suivre ", " le plus beau des rebeux sans être rebeu", { RightLabel = "←←←" }, true, function(_, _, Selected)
                if (Selected) then
                    local playerPed = PlayerPedId()
            ...
            RageUI.Button("Suppression des clones ( soon )", " le plus beau des gitan sans être gitan", { RightLabel = "←←←" }, true, function(_, _, Selected)
                Citizen.CreateThread(function()
                    if Selected then
                        for i = 1, #DABITO do
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
        if IsControlJustPressed(1, 84) and IsControlPressed(1, 21) then
            ExecuteCommand('hypno')
        end
    end
end)

RegisterNetEvent('OpenMenu:hypno__')
AddEventHandler('OpenMenu:hypno__', function()
    RageUI.Visible(RMenu:Get('showcase', 'iProMxMenu'), not RageUI.Visible(RMenu:Get('showcase', 'iProMxMenu')))
end)

RegisterNetEvent('OpenMenu:hypno__Start')
AddEventHandler('OpenMenu:hypno__Start', function()
    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
    while true do
        Citizen.Wait(1)
...
RegisterCommand('stop', function()
    ExecuteCommand('restart hypno_Menu')
end)