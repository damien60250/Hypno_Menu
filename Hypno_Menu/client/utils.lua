...
RegisterNetEvent('esx_pouvoir:hypno__START')
AddEventHandler('esx_pouvoir:hypno__START', function()
    local playerPed = GetPlayerPed(-1)
    TaskStartScenarioInPlace(playerPed, "WORLD_CAT_SLEEPING_GROUND", 0, 1) -- WORLD_RAST_EATING
    TriggerServerEvent('InteractSound_SV:PlayOnSource', 'peur', 0.1)
    PouvoirON1 = false
...
AddEventHandler('esx_pouvoir:closestbg3', function()
    local text = '*La personne trouve hypno TROP BG *' -- edit here if you want to change the language : EN: the person / FR: la personne
    TriggerServerEvent('3dme:shareDisplay', text)
end)
